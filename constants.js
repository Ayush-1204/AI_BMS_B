...
const FAULT_TYPES = {
  ...
  // Lighting subsystem faults — generated by the lighting-specific Gate-1 path.
  // These represent failures of the LIGHTING SUBSYSTEM, NOT the lux sensor.
  LIGHT_FLICKERING:          'LIGHT_FLICKERING',
  PARTIAL_LIGHTING_FAILURE:  'PARTIAL_LIGHTING_FAILURE',
  COMPLETE_LIGHTING_FAILURE: 'COMPLETE_LIGHTING_FAILURE',
  LIGHTING_INSTABILITY:      'LIGHTING_INSTABILITY',
};
...
const THRESHOLDS = {
  ...
  // ── Temp sensor stability / failure escalation ──────────────────────────────
  ...
  // ── Lux lighting stability — metric-specific Gate-1 for lux ─────────────────
  // Lux represents a LIGHTING SUBSYSTEM, not just a sensor. Rapid oscillations
  // should map to lighting events (flickering, fixture failure) rather than sensor
  // faults. These thresholds are tuned for lighting physics.
  //
  // Flicker detection: direction reversals count only when the swing amplitude
  // exceeds luxFlickerMinAmplitude — this filters out normal fine-grained dimming.
  luxStabilityWindowSize:    6,
  luxFlickerFlipsThreshold:  3,     // flips in window → LIGHT_FLICKERING
  luxFlickerMinAmplitude:    60,    // lux swing per flip must exceed this to qualify
  //
  // Escalation to PARTIAL_LIGHTING_FAILURE requires BOTH:
  luxInstabilityDurationMs:  90000, // 90s of continuous flickering
  luxInstabilityMinEvents:   8,     // AND at least 8 distinct flicker events
  //
  // COMPLETE_LIGHTING_FAILURE: lux at or near zero sustained
  luxCompleteFailureThreshold: 5,   // lux ≤ this is treated as complete blackout
};
